...
import {
  insightStatusConfig,
  insightStatusOrder,
  type UIInsightStatus,
} from "@/lib/ui-config"
import type { LifecycleInsight, InsightMessage, FeatureRow, UserInterviewRow } from "@/lib/types"
import {
  Sparkles,
  Send,
  User,
...
const statusSteps: { key: UIInsightStatus; short: string }[] = [
...
function LifecycleProgress({ status }: { status: UIInsightStatus }) {
...
function RecommendationCard({ rec, status }: { rec: NonNullable<InsightMessage["recommendation"]>; status: UIInsightStatus }) {
...
interface AnalysisClientProps {
  ...
  features: FeatureRow[]
  userInterviews: UserInterviewRow[]
  iterationLoop: ReactNode
}

export function AnalysisClient({ lifecycleInsights, features, userInterviews, iterationLoop }: AnalysisClientProps) {
  const [selectedId, setSelectedId] = useState(lifecycleInsights[1]?.id ?? lifecycleInsights[0]?.id)
  ...
  const pipelineCounts = insightStatusOrder.reduce((acc, s) => {
    ...
    return acc
  }, {} as Record<UIInsightStatus, number>)
