defmodule SymphonyElixir.Orchestrator do
  ...
  @impl true
  def init(opts) do
    workflow_store_name = Keyword.get(opts, :workflow_store_name, SymphonyElixir.WorkflowStore)
    project_config = resolve_project_config(workflow_store_name)
    now_ms = System.monotonic_time(:millisecond)

    state = %State{
      project_config: project_config,
      workflow_store_name: workflow_store_name,
  ...
  defp resolve_project_config(workflow_store_name) do
    case SymphonyElixir.WorkflowStore.current(workflow_store_name) do
      {:ok, workflow} ->
  ...
  def handle_info({:retry_issue, issue_id}, state) do
    ...
    case result do
      {:noreply, updated_state} -> notify_dashboard(updated_state)
  ...
  defp maybe_dispatch(%State{} = state) do
    ...
    with :ok <- Config.validate!(),
         {:ok, issues} <- fetch_candidate_issues_for_state(state),
  ...
  defp reconcile_running_issues(%State{} = state) do
    ...
    if running_ids == [] do
      ...
      case fetch_issue_states_by_ids_for_state(running_ids, state) do
        {:ok, issues} ->
  ...
  defp handle_retry_issue(%State{} = state, issue_id, attempt, metadata) do
    case fetch_candidate_issues_for_state(state) do
      {:ok, issues} ->
        issues
  ...
  defp run_terminal_workspace_cleanup(%ProjectConfig{} = project_config) do
    Task.Supervisor.start_child(SymphonyElixir.TaskSupervisor, fn ->
      do_terminal_workspace_cleanup(project_config)
    end)
  end

  defp do_terminal_workspace_cleanup(project_config) do
    fetch_result =
      if is_binary(project_config.linear_api_token) and is_binary(project_config.linear_project_slug) do
        Client.fetch_candidate_issues_for_project(
          project_config.linear_api_token,
          project_config.linear_project_slug,
          project_config.linear_terminal_states
        )
      else
        Tracker.fetch_issues_by_states(project_config.linear_terminal_states)
      end
      ...
      {:ok, issues} ->
        Enum.each(issues, fn
          %Issue{identifier: identifier} when is_binary(identifier) ->
            Workspace.remove_issue_workspaces(identifier, project_config: project_config)
  ...
  defp notify_dashboard(%State{} = state) do
    ...
    project_id = build_project_id(state)
    now = DateTime.utc_now()
    now_ms = System.monotonic_time(:millisecond)

    snapshot = %{
      project_id: project_id,
      running: serialize_running(state.running, now),
      retrying: serialize_retrying(state.retry_attempts, now_ms),
    ...
    StatusDashboard.update(snapshot)
  end

  defp build_project_id(%State{workflow_store_name: store_name}) do
    store_name
    |> to_string()
    |> String.replace(~r/^Elixir\./, "")
    |> String.replace("SymphonyElixir.WorkflowStore.", "")
    |> String.replace("SymphonyElixir.WorkflowStore", "default")
  end

  defp serialize_running(running_map, now) do
    Enum.map(running_map, fn {issue_id, metadata} ->
  ...
  defp serialize_retrying(retry_attempts, now_ms) do
  ...
  defp refresh_runtime_config(%State{workflow_store_name: store_name} = state) do
    case SymphonyElixir.WorkflowStore.current(store_name) do
      {:ok, workflow} ->
  ...
  defp fetch_issue_states_by_ids_for_state(issue_ids, %State{project_config: project_config}) do
    if is_binary(project_config.linear_api_token) do
