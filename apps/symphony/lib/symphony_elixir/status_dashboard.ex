defmodule SymphonyElixir.StatusDashboard do
  ...
  @doc """
  Stores a per-project snapshot for multi-project aggregation.
  The snapshot map must include a `project_id` key. If absent, `:default` is used.
  """
  @spec update(map()) :: :ok
  def update(snapshot) when is_map(snapshot) do
    case GenServer.whereis(__MODULE__) do
      pid when is_pid(pid) ->
        GenServer.cast(__MODULE__, {:update_project_snapshot, snapshot})
  ...
  @doc """
  Returns a merged snapshot across all projects. For single-project mode this is
  equivalent to the single project's snapshot. For multi-project mode, running and
  retrying lists are concatenated and codex_totals are summed.
  """
  @spec snapshot() :: map() | nil
  def snapshot do
    case GenServer.whereis(__MODULE__) do
      pid when is_pid(pid) ->
        GenServer.call(__MODULE__, :merged_snapshot)
  ...
  def reset do
    case GenServer.whereis(__MODULE__) do
      pid when is_pid(pid) -> GenServer.call(__MODULE__, :reset_snapshots)
      _ -> :ok
    end
  end
  ...
  @impl true
  def handle_cast({:update_project_snapshot, snapshot}, state) do
    project_id = Map.get(snapshot, :project_id) || :default
    snapshots = Map.put(state.snapshots, project_id, snapshot)
    {:noreply, %{state | snapshots: snapshots}}
  end

  @impl true
  def handle_call(:merged_snapshot, _from, state) do
    merged = merge_project_snapshots(Map.values(state.snapshots))
    {:reply, merged, state}
  end

  def handle_call(:reset_snapshots, _from, state) do
    {:reply, :ok, %{state | snapshots: %{}}}
  end

  defp merge_project_snapshots([]), do: nil
  ...
  defp merge_project_snapshots(snapshots) do
    ...
    codex_totals =
      Enum.reduce(snapshots, %{input_tokens: 0, output_tokens: 0, total_tokens: 0, seconds_running: 0}, fn snap, acc ->
        totals = Map.get(snap, :codex_totals, %{})
        %{
    ...
    rate_limits =
      snapshots
      |> Enum.map(&Map.get(&1, :codex_rate_limits))
  ...
  defp maybe_render(state) do
    now_ms = System.monotonic_time(:millisecond)
    {snapshot_data, token_samples} = snapshot_with_samples(state.token_samples, now_ms, state)
  ...
  defp snapshot_with_samples(token_samples, now_ms, state) do
    case snapshot_payload(state) do
  ...
  defp snapshot_payload(state) do
    ...
    if not is_nil(merged) and is_list(merged.running) and is_list(merged.retrying) do
      {:ok,
       %{
         running: merged.running,
         retrying: merged.retrying,
         codex_totals: merged.codex_totals,
         # merged view doesn't aggregate rate_limits meaningfully
         rate_limits: nil,
         polling: nil
       }}
    else
      # Fall back to single Orchestrator for backward compat
      if Process.whereis(Orchestrator) do
        case Orchestrator.snapshot() do
          %{running: running, retrying: retrying, codex_totals: codex_totals} = snap
          when is_list(running) and is_list(retrying) ->
            {:ok,
             %{
               ...
               retrying: retrying,
               codex_totals: codex_totals,
               rate_limits: Map.get(snap, :rate_limits),
