defmodule SymphonyElixir.Config do
  ...
  All env var references ($VAR) are expanded and defaults applied.
  """
  ...
  def resolve(workflow_config) when is_map(workflow_config) do
    ...
    options = normalized |> extract_workflow_options() |> NimbleOptions.validate!(@workflow_options_schema)
    hooks = options[:hooks]
    ...
    %SymphonyElixir.ProjectConfig{
      ...
      workspace_root: resolve_path_value(options[:workspace][:root], @default_workspace_root),
      workspace_hooks: %{
      ...
      codex_approval_policy: resolve_approval_policy_from_config(normalized),
      codex_thread_sandbox: resolve_thread_sandbox_from_config(normalized),
  ...
  @doc """
  Resolves a loaded workflow (from WorkflowStore.current/0) into a %ProjectConfig{}.
  """
  ...
  defp resolve_approval_policy_from_config(config) do
    case resolve_config_value(config, [["codex", "approval_policy"]]) do
      :missing -> @default_codex_approval_policy
      nil -> @default_codex_approval_policy
      value when is_binary(value) ->
        trimmed = String.trim(value)
        if trimmed == "", do: @default_codex_approval_policy, else: trimmed
      value when is_map(value) -> value
      _ -> @default_codex_approval_policy
    end
  end

  defp resolve_thread_sandbox_from_config(config) do
    case resolve_config_value(config, [["codex", "thread_sandbox"]]) do
      :missing -> @default_codex_thread_sandbox
      nil -> @default_codex_thread_sandbox
      value when is_binary(value) ->
        trimmed = String.trim(value)
        if trimmed == "", do: @default_codex_thread_sandbox, else: trimmed
      _ -> @default_codex_thread_sandbox
    end
  end
