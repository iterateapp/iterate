defmodule SymphonyElixir.Workspace do
  ...
  def create_for_issue(issue_or_identifier, opts \\ []) do
    ...
    try do
      ...
      workspace = workspace_path_for_issue(safe_id, opts)

      with :ok <- validate_workspace_path(workspace, opts),
  ...
  def run_before_run_hook(workspace, issue_or_identifier, opts \\ []) when is_binary(workspace) do
    issue_context = issue_context(issue_or_identifier)
    hooks = config_workspace_hooks(opts)
  ...
  defp workspace_path_for_issue(safe_id, opts \\ []) when is_binary(safe_id) do
  ...
  defp validate_workspace_path(workspace, opts \\ []) when is_binary(workspace) do
    ...
    root = Path.expand(config_workspace_root(opts))
    root_prefix = root <> "/"
