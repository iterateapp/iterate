defmodule SymphonyElixir.Linear.Client do
  ...
  @spec fetch_candidate_issues_for_project(String.t() | nil, String.t() | nil, [String.t()]) ::
          {:ok, [Issue.t()]} | {:error, term()}
  def fetch_candidate_issues_for_project(nil, _project_slug, _states),
    do: {:error, :missing_linear_api_token}

  def fetch_candidate_issues_for_project(_token, nil, _states),
    do: {:error, :missing_linear_project_slug}

  def fetch_candidate_issues_for_project(api_token, project_slug, active_states)
      when is_binary(api_token) and is_binary(project_slug) do
    request_fun = make_request_fun_for_token(api_token)
    do_fetch_by_states(project_slug, active_states, nil, request_fun: request_fun)
  end

  @spec fetch_issue_states_by_ids_for_project([String.t()], String.t() | nil) ::
          {:ok, [Issue.t()]} | {:error, term()}
  def fetch_issue_states_by_ids_for_project([], _api_token), do: {:ok, []}

  def fetch_issue_states_by_ids_for_project(_ids, nil),
    do: {:error, :missing_linear_api_token}

  def fetch_issue_states_by_ids_for_project(issue_ids, api_token)
      when is_list(issue_ids) and is_binary(api_token) do
    request_fun = make_request_fun_for_token(api_token)
    ids = Enum.uniq(issue_ids)
    do_fetch_issue_states(ids, nil, request_fun: request_fun)
  end

  defp make_request_fun_for_token(api_token) do
    fn payload, _default_headers ->
      headers = [{"Authorization", api_token}, {"Content-Type", "application/json"}]
      post_graphql_request(payload, headers)
    end
  end
