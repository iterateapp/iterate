defmodule SymphonyElixir.Codex.AppServer do
  ...
  def start_session(workspace, opts \\ []) do
    with :ok <- validate_workspace_cwd(workspace),
      ...
      with {:ok, session_policies} <- config_session_policies(expanded_workspace, opts),
           {:ok, thread_id} <- do_start_session(port, expanded_workspace, session_policies, config_codex_read_timeout_ms(opts)) do
        {:ok,
         %{
           port: port,
           metadata: metadata,
  ...
  def run_turn(
    ...
    case start_turn(port, thread_id, prompt, issue, workspace, approval_policy, turn_sandbox_policy) do
      {:ok, turn_id} ->
        ...
        case await_turn_completion(port, on_message, tool_executor, auto_approve_requests, config_codex_turn_timeout_ms(opts)) do
          {:ok, result} ->
      ...
      port =
        Port.open(
          ...
          [
            ...
            :exit_status,
            :stderr_to_stdout,
            args: [~c"-lc", String.to_charlist(config_codex_command(opts))],
  ...
  defp send_initialize(port, read_timeout_ms \\ nil) do
    payload = %{
      "method" => "initialize",
      "id" => @initialize_id,
      "params" => %{
  ...
  defp config_codex_turn_timeout_ms(opts) do
    case Keyword.get(opts, :project_config) do
      %{codex_turn_timeout_ms: ms} when is_integer(ms) -> ms
      _ -> Config.codex_turn_timeout_ms()
    end
  end

  defp config_codex_read_timeout_ms(opts) do
    case Keyword.get(opts, :project_config) do
      %{codex_read_timeout_ms: ms} when is_integer(ms) -> ms
      _ -> Config.codex_read_timeout_ms()
    end
  end

  defp config_codex_stall_timeout_ms(opts) do
    case Keyword.get(opts, :project_config) do
      %{codex_stall_timeout_ms: ms} when is_integer(ms) -> ms
      _ -> Config.codex_stall_timeout_ms()
    end
  end

  defp config_session_policies(workspace, opts) do
    case Keyword.get(opts, :project_config) do
      %{codex_approval_policy: policy, codex_thread_sandbox: sandbox}
      when not is_nil(policy) and not is_nil(sandbox) ->
        {:ok, %{
          approval_policy: policy,
          thread_sandbox: sandbox,
          turn_sandbox_policy: Config.codex_turn_sandbox_policy(workspace)
        }}
      _ ->
  ...
  defp do_start_session(port, workspace, session_policies, read_timeout_ms \\ nil) do
    effective_read_timeout_ms = read_timeout_ms || Config.codex_read_timeout_ms()

    case send_initialize(port, effective_read_timeout_ms) do
      :ok -> start_thread(port, workspace, session_policies, effective_read_timeout_ms)
      {:error, reason} -> {:error, reason}
    end
  end

  defp start_thread(port, workspace, %{approval_policy: approval_policy, thread_sandbox: thread_sandbox}, read_timeout_ms \\ nil) do
    send_message(port, %{
      "method" => "thread/start",
      "id" => @thread_start_id,
      "params" => %{
  ...
  defp await_turn_completion(port, on_message, tool_executor, auto_approve_requests, turn_timeout_ms) do
    receive_loop(port, on_message, turn_timeout_ms, "", tool_executor, auto_approve_requests)
  end
  ...
  defp await_response(port, request_id, read_timeout_ms \\ nil) do
    effective_timeout_ms = read_timeout_ms || Config.codex_read_timeout_ms()
    with_timeout_response(port, request_id, effective_timeout_ms, "")
  end
