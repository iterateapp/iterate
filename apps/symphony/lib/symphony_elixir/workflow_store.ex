defmodule SymphonyElixir.WorkflowStore do
  ...
  @spec current(GenServer.name()) :: {:ok, Workflow.loaded_workflow()} | {:error, term()}
  def current(name) do
  ...
  @spec force_reload(GenServer.name()) :: :ok | {:error, term()}
  ...
  @impl true
  def init(opts) do
    explicit_path = Keyword.get(opts, :workflow_path)
    path = explicit_path || Workflow.workflow_file_path()
    fixed = !is_nil(explicit_path)

    case load_state(path) do
      {:ok, state} ->
        schedule_poll()
        {:ok, %{state | fixed_path: fixed}}
  ...
  defp reload_state(%State{fixed_path: true} = state) do
    # Per-project store: path is fixed at init, always use state.path
    reload_current_path(state.path, state)
  end

  defp reload_state(%State{fixed_path: false} = state) do
    # Default store: support hot-swapping the global workflow path
